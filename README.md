...
> **你能学会吗？**  
> 能。你不需要会编程。这份文档按「第一次用 Mac 做研究采集」来写：先讲名词，再讲点哪里、敲什么、成功时长什么样。  
> **请严格按章节顺序做。** 不要跳去学术采样，除非你至少完成过一次「新建任务 + 登录 + 看到采集结果」。
...
9. [学术采样：视频转写 + 评论树 + 滚雪球](#9-学术采样视频转写--评论树--滚雪球)
...
| 流水线 | 你什么时候用 | 比喻 |
| --- | --- | --- |
| **新建任务** | 已知几个 UP 主，要把他们一段时间里的投稿「整库搬下来」 | 给指定作者建档案馆 |
| **学术采样** | 从几条「像研究对象」的视频出发，沿「相关推荐」扩散，并筛掉游戏鬼畜等噪声 | 在推荐算法里滚雪球抽样 |

两条线**共存**，不会互相替换。学术采样里通过门禁的视频，会同时采 **视频台词** 和 **评论树**——评论和视频话语是两层分析单位，不要混成一张表做主题模型。
...
| **门禁** | 学术采样里的筛选规则。只有同时满足分区、播放量、互动、标签的视频才会继续沿推荐往外滚。 |
...
**成功：** 看到左侧深色菜单：使用指南、新建任务、学术采样、运行监控、采集结果、分析数据集、设置与登录。
...
**页面缺「学术采样」等菜单：** 硬刷新（Command + Shift + R）。
...
给 **一个或多个 UP 主 UID**，按时间范围采视频、动态、评论、字幕等。适合「档案馆」式采集。

### 5.3 学术采样

给 **种子视频或 BV 号**，沿相关推荐滚雪球，带门禁筛选。适合论文抽样。默认同时采 **视频转写** 和 **评论树**。

### 5.4 运行监控

任务开始后自动跳转到这里。看进度条、数字、黑底日志。可「取消任务」。
...
**不登录也能跑，但你会吃亏：** 评论少、没有评论 IP 属地、字幕和空间列表更容易失败。学术采样和完整归档**务必登录**。
...
目标：确认「登录 → 开任务 → 看监控 → 看结果」整条链路是通的。**先不要开学术采样，先不要开 Whisper。**
...
**干什么：** 「文字研究 / 完整归档 / 学术采样开 Whisper」会在本机留下大音频或 MP4。转写文字已经进数据库了，原片可以上传 Google Drive 后**删除本地**，省硬盘。
...
5. 日志显示就绪后，新建任务步骤 6 或学术采样步骤 4 选 **云端 GPU**。
...
1. 点 **开始滚雪球** → 自动到 **运行监控**。
2. 日志会出现：`滚雪球启动`、`剪枝 BVxxxx · category/tags/engagement`、`xxx 相关 N · 新入队 M`、`视频完成`。
3. **剪枝很多是正常的**——说明门禁在挡噪声。
4. 连续 **5 次风控** 会自动停，队列保留；改天同一套种子再点开始可续跑。
...
若你**先**用「新建任务」采过一堆，**后**才要用 SQLite：点 **回填现有档案**，程序会扫描 `data/library/` 灌进 `corpus.db`（可能要几分钟）。
...
| 学术采样全剪枝 | 种子或门禁太严 | 看 `v_gate_funnel`；调标签/白名单/试跑降阈值 |
...
第 2 天：9 学术采样（只官方字幕试跑）→ 10 看漏斗和 SQL
